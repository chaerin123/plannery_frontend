...
export default function WeekCalendar({
  ...
  const handleDatePress = (date: Date) => {
    // 선택한 날짜를 시작일로 하고, 그 날부터 7일 후를 종료일로 설정
    const startDate = new Date(date);
    startDate.setHours(0, 0, 0, 0);
    
    const endDate = new Date(startDate);
    endDate.setDate(startDate.getDate() + 6); // 시작일 포함 7일
          ...
          {days.map((date, index) => {
            ...
            return (
              <TouchableOpacity
                ...
                style={[
                  styles.dayCell,
                  inSelectedWeek && styles.selectedWeekCell,
                  isStart && styles.weekStartCell,
                  isEnd && styles.weekEndCell,
                ]}
              ...
              >
                <Text
                  style={[
                    styles.dayText,
                    inSelectedWeek && styles.selectedWeekText,
                    (isStart || isEnd) && styles.weekStartEndText,
